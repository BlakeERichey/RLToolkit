  ...
  def train(self, env, generations, elites=None, 
    ...
    for gen in range(generations):
      ...
      for i, colony in enumerate(self.colonies):
        ...
        ranked.append((i, res, val)) #should replace with named tuple
      ...
      if goal:
        if not validate:
          ...
          goal_met = ranked[0][1]>=goal and ranked[0][2]>=goal
      
      #next gen
      if gen != generations - 1 and not(goal_met):
        
        elite_ids = [ranked[i][0] for i in range(elites)]
        ...
        for i, colony in enumerate(mating_pool):
          if len(new_colonies) < remake:
            parent1 = self.colonies[colony[0]] #colony = (index, res, val)
